...
uint8 osal_init_system( void )
{
  ...
  osal_mem_init();//³õÊ¼»¯ÄÚ´æ·ÖÅäÏµÍ³
#endif /* !defined USE_ICALL && !defined OSAL_PORT2TIRTOS */

  // Initialize the message queue
  osal_qHead = NULL;//³õÊ¼»¯ÏûÏ¢¶ÓÁÐ  ÈÎÎñÖ®¼äµÄÍ¨ÐÅ¿¿µÄ¾ÍÊÇÏûÏ¢¶ÓÁÐ

  // Initialize the timers
  osalTimerInit(); //³õÊ¼»¯¶¨Ê±Æ÷

  // Initialize the Power Management System
  osal_pwrmgr_init(); //³õÊ¼»¯µçÔ´¹ÜÀíÏµÍ³
  ...
  osalInitTasks(); //³õÊ¼»¯ÏµÍ³ÈÎÎñ£¬ÖØµã¹Ø×¢

#if !defined USE_ICALL && !defined OSAL_PORT2TIRTOS
  // Setup efficient search for the first free block of heap.
  osal_mem_kick(); //ÉèÖÃÓÐÐ§µÄ²éÕÒ¶ÑÉÏµÄµÚÒ»¸ö¿ÕÏÐ¿é
#endif /* !defined USE_ICALL && !defined OSAL_PORT2TIRTOS */
