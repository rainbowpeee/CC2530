...
void osalInitTasks( void )
{
  uint8 taskID = 0;
  /*
  osal_mem_alloc()¸Ãº¯ÊýÊÇOSALÖÐµÄÄÚ´æ¹ÜÀíº¯Êý£¬
  ÊÇÒ»¸ö´æ´¢·ÖÅäº¯Êý£¬·µ»ØÖ¸ÏòÒ»¸ö»º´æµÄÖ¸Õë£¬
  ²ÎÊýÊÇ±»·ÖÅä»º´æµÄ´óÐ¡£¬ÆätasksCntµÄ¶¨ÒåÈçÏÂ
  const uint8tasksCnt = sizeof( tasksArr ) / sizeof( tasksArr[0] );
  tasksEventsÖ¸Ïò±»·ÖÅäµÄÄÚ´æ¿Õ¼ä£¬ÕâÀï×¢Òâ
  tasksArr[]º¯ÊýÖ¸ÕëÊý×éµÄÁªÏµÊÇÒ»Ò»¶ÔÓ¦µÄ¡£
  tasksEvents¾ÍÊÇÖ¸ÏòµÚÒ»¸ö±»·ÖÅäµÄÈÎÎñµÄÄÚ´æ¿Õ¼ä
*/
  tasksEvents = (uint16 *)osal_mem_alloc( sizeof( uint16 ) * tasksCnt);
  
  //°ÑÉêÇëµÄÄÚ´æ¿Õ¼äÈ«²¿ÉèÖÃÎª0£¬tasksCntÈÎÎñÊý * µ¥¸öÈÎÎñÕ¼µÄÄÚ´æ¿Õ¼ä£¨4byte£©
  osal_memset( tasksEvents, 0, (sizeof( uint16 ) * tasksCnt));
  //ÏÂÃæ¾ÍÊÇZ-StackÐ­ÒéÕ»ÖÐ£¬´ÓMAC²ãµ½ZDO²ãµÄ³õÊ¼»¯º¯Êý£¬ÆäÖÐµÄ²ÎÊý¶¼ÊÇÈÎÎñµÄID£¬²»¹ýIDºÅÊÇÒÀ´ÎµÝÔöµÄ
