...
//    printDebugString("PA\n");
  ...
  OLED_Init();//³õÊ¼»¯OLED  
   OLED_Clear();
   OLED_ShowString(20,0,"SMM system",16);
   OLED_ShowString(0,2,"SH:",16);  
   OLED_ShowString(0,4,"SET",16);
   OLED_ShowString(0,6,"H:",16);  
   OLED_ShowString(63,6,"L:",16);  
}
...
void appSample_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  t = pkt->cmd.Data[0]*10+pkt->cmd.Data[1];
  OLED_ShowNum(41,2,t,3,16);//ÏÔÊ¾ASCII×Ö·ûµÄÂëÖµ 	
...
void serialCallback( uint8 port, uint8 events )  //´®¿ÚÊÕµ½ÏûÏ¢´¦Àí
{
  ...
  unsigned char SET_t[2];
  if (events & ( HAL_UART_RX_FULL | HAL_UART_RX_ABOUT_FULL | HAL_UART_RX_TIMEOUT))
  {
    SampleApp_TxLen = HalUARTRead(0,SampleApp_TxBuf,80); 
    if((SampleApp_TxBuf[0] == 'a')&&(SampleApp_TxBuf[1] == 'b'))
    {
      SET_t[0] = (SampleApp_TxBuf[2] -48) *10 + (SampleApp_TxBuf[3] -48);
      SET_t[1] = (SampleApp_TxBuf[4] -48) *10 + (SampleApp_TxBuf[5] -48);  
      OLED_ShowNum(25,6,SET_t[0],2,16);//ÏÔÊ¾ASCII×Ö·ûµÄÂëÖµ 

        OLED_ShowNum(80,6,SET_t[1],2,16);//ÏÔÊ¾ASCII×Ö·ûµÄÂëÖµ 
