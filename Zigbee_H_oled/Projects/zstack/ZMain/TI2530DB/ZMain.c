...
{
  // Turn off interrupts
  osal_int_disable( INTS_ALL );//πÿ±’÷–∂œ

  // Initialization for board related stuff such as LEDs
  HAL_BOARD_INIT();//≥ı ºªØ”≤º˛

  // Make sure supply voltage is high enough to run
  zmain_vdd_check(); //µÁ—πºÏ≤‚£¨»∑±£–æ∆¨ƒ‹’˝≥£π§◊˜µƒµÁ—π

  // Initialize board I/O
  InitBoard( OB_COLD ); //≥ı ºªØ∞Â‘ÿI/O

  // Initialze HAL drivers
  HalDriverInit(); //≥ı ºªØ”≤º˛«˝∂Ø

  // Initialize NV System
  osal_nv_init( NULL ); //≥ı ºªØNVœµÕ≥

  // Initialize the MAC
  ZMacInit(); //≥ı ºªØMAC

  // Determine the extended address
  zmain_ext_addr();//»∑∂®¿©’πµÿ÷∑£®64ŒªIEEE/ŒÔ¿Ìµÿ÷∑£©
  ...
  osal_init_system(); //≥ı ºªØ≤Ÿ◊˜œµÕ≥

  // Allow interrupts
  osal_int_enable( INTS_ALL ); // πƒ‹÷–∂œ

  // Final board initialization
  InitBoard( OB_READY ); //◊Ó÷’∞Â‘ÿ≥ı ºªØ

  // Display information about this device
  zmain_dev_info(); //œ‘ æ…Ë±∏–≈œ¢
  ...
  osal_start_system(); // No Return from here√ª”–∑µªÿ£¨º¥Ω¯»Î≤Ÿ◊˜œµÕ≥

  return 0;  // Shouldn't get here.//≤ªª·‘À––µΩ’‚
} // main()
